...
public class MissingImplStrategyFactory extends AbstractProxyStrategyFactory {
	...
	Object handle(Class<?> apiClass, Method method, Object[] args) throws Throwable {
		...
		List<Throwable> problems = Environment.getStrategyList(apiClass).strategyHolders().stream()
				.filter((h) -> (h.getUnavailableCause() != null)).map(StrategyHolder::getUnavailableCause)
				.collect(Collectors.toList());

		if (problems.size() == 1) {
			throw problems.get(0);
		} else if (problems.size() > 1) {
			throw new UnsupportedOperationException("Multiple problems. TODO! enhance code to display more information.");
		}
